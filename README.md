...
如果你想将工具打包为 .exe 或 .dmg 直接使用或分发给朋友：
...
| 功能 | 快捷键 |
|---|---|
| **显示 / 隐藏助手** | `Ctrl + Shift + H` |
| **切换鼠标点击穿透** | `Ctrl + Shift + T` |
| **增加透明度 (变清晰)** | `Ctrl + Shift + Up (上方向键)` |
| **降低透明度 (变透明)** | `Ctrl + Shift + Down (下方向键)` |
...
├── src/
│   ├── main/               # 后端逻辑: 窗口、快捷键、系统托盘管理等
│   ├── renderer/           # 前端渲染: 核心 UI、Webview 处理、前端逻辑
│   ├── common/             # 公共库: 共享常量与多端共享工具函数(i18n)
