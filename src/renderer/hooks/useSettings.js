...
function useDebouncedSave(callback, delay = 500) {
...
function useSettingsSearch(allItems) {
...
export function useSettingsPanel(locale, onLocaleChange, showOSD, t) {
  ...
  const handleAutoStartChange = useCallback(async (checked) => {
    setAutoStart(checked);
    await window.wingman.settings.setAutoStart(checked);
  }, []);

  const handleStartupPageChange = useCallback(async (value) => {
  ...
  const handleCloseStrategyChange = useCallback(async (value) => {
    setCloseStrategy(value);
    await window.wingman.settings.setCloseStrategy(value);
  }, []);

  const handleRememberBoundsChange = useCallback(async (checked) => {
